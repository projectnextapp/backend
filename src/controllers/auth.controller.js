...
exports.createGroup = async (req, res) => {
  try {
    const { name, location, contactEmail, password, memberSizeRange, adminInfo } = req.body;
...
exports.memberLogin = async (req, res) => {
  try {
    ...
    if (!email || !password || !groupId) {
      return res.status(400).json({ success: false, message: 'Email, password, and group ID are required' });
    }

    const member = await Member.findOne({ email, group: groupId }).select('+password');
    if (!member) {
      return res.status(401).json({ success: false, message: 'Invalid credentials' });
    }

    if (member.status === 'pending') {
      return res.status(403).json({ success: false, message: 'Your membership is pending approval' });
    }
